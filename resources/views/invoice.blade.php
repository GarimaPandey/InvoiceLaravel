...
<div class="container-fluid">
     ...
     <div class="row" >
      ...
      <div class = "alert alert-danger">
       ...
       @endif
            ...
            @if($data)
            <!-- Large modal -->
        <p align="right"><button type="button" class="Button1-Default" data-toggle="modal" data-target=".bd-example-modal-md">
          <svg id="plus_icon"width="11" height="11" xmlns="http://www.w3.org/2000/svg"><path d="M6.313 10.023v-3.71h3.71v-2.58h-3.71V.023h-2.58v3.71H.023v2.58h3.71v3.71z" fill="#7C5DFA" fill-rule="nonzero"/></svg>New Invoice </button>
        </p>
        <div class="modal fade bd-example-modal-md" tabindex="+2" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
            <div class="modal-content">
              <div class="row">
                <div class="container">
              <form action="create" name="InvoiceForm" method="POST">
                @csrf 
               
                <div class="row" style="align-right"><h2>New Invoice</h2></div>
                ...
                <div class="row">
                  <div class="form-group col-md-9">
                    ...
                    <input type="text" class="form-control" name="sender_street_address" id="sender_street_address">
                  </div>
                </div>
                <div class="row">
                ...
                <div class="form-group col-md-3">
                  ...
                  <input type="text" class="form-control" name="sender_post_code" id="sender_post_code">
                </div>
                <div class="form-group col-md-3">
                ...
                <div class="row">
                  <div class="form-group col-md-9">
                    ...
                    <input type="text" class="form-control" name="client_street_address" id="client_street_address">
                  </div>
                </div>
                <div class="row">
                  <div class="form-group col-md-3">
                  ...
                  <div class="form-group col-md-3">
                    ...
                    <input type="text" class="form-control" name="client_post_code" id="client_post_code">
                  </div>
                  <div class="form-group col-md-3">
                ...
                <table class="new_item" id="addItem">
                  ...
                  <tr> 
                    <td id="col0"><input type="text" class="form-control" name="itemname[]" value="" /></td> 
                    <td id="col1"><input type="text" class="form-control" id="qty" name="qty[]" onkeyup="newFunction()" value=""/></td> 
                    <td id="col2"><input type="text" class="form-control" id="price" name="price[]" onkeyup="newFunction()" value=""/></td> 
                    <td id="col3"><input type="text" class="form-control" id="total" name="total[]" value="" /></td> 
                    <td id="col4"><input type="button" class="delete" value="Delete Row" onclick="deleteRows()" readonly/></td> 
                  </tr>  
                </table> 
                <table> 
                  <tr> 
                    <td><input type="button" class="Button1-Default" value="Add Row" onclick="addRows()" /></td> 
                    <td><button type="submit" name="action" value="save" class="Button1-Default" >Save and Send</button></td>
                    <td><button type="submit" name="action" value="draft" class="Button1-Default" >Save as Draft</button></td>
                    <td><input type="reset" value="Reset"  class="delete"></td>
                    <td><button class="delete" type="button" onclick="window.location='{{ url("/") }}'">Cancel</button>
                    </td>
                  </tr>  
                </table> 
                  </div>
  ...
  <tbody>
    @foreach($data as $key=>$value) 
      <tr>
          <td>
          <h5>#<b>{{$value->invoice_id}}</b></h5>
          </td>
          <td>
          <h5> Due {{ date('d M Y', strtotime($value->paymentDue)) }}</h5>
          </td>
          <td>
             <h5>{{$value->clientName}}</h5>
          </td>
          <td>
           <h4><b>${{$value->total}}</b></h4>
          </td>
          <td>
            <?php if($value->status == 'paid'){  ?>
            <h5><span class ="paid-pill"> {{$value->status}}</span></h5>
            <?php } elseif($value->status == 'pending') { ?>
            <h5><span class ="pending-pill"> {{$value->status}}</span></h5>
            <?php } else { ?>
            <h5><span class ="pill"> {{$value->status}}</span></h5>
            <?php } ?>
          </td>
          <td>
            <a href="{{ url('/view/'.$value->invoice_id )}}"><svg width="50" height="50" xmlns="http://www.w3.org/2000/svg"><path d="M1 1l4 4-4 4" stroke="#7C5DFA" stroke-width="2" fill="none" fill-rule="evenodd"/></svg></a>
           </td>
...
//      if(total.length !== 0 )
//      {
//       product *=parseFloat(total);
//      }
//     });
//     $(this).find('#final_product').html('= '+product);
//   });
// };
...
function addRows(){ 
    ...
    $("table.new_item").on("change", 'input[name^="price"], input[name^="qty"]', function (event) {
